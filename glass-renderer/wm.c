...
Bool main_event_handler_function(EventHandler *handler, XEvent *event) {
  ...
  WITH_LOCK(&global_lock) {
    ...
    if (event->type == PropertyNotify) {
      PropertyFetch *fetch = malloc(sizeof(PropertyFetch));
      fetch->win = event->xproperty.window;
      fetch->name = event->xproperty.atom;
      property_updater_submit(fetch);
      
      if ((event->xproperty.window == xconn->root) && (event->xproperty.atom == ATOM(xconn, "IG_SHADERS"))) {
