...
/*
if (texture_coord.x < 0.01 || texture_coord.x > 0.99
    || texture_coord.y < 0.01 || texture_coord.y > 0.99) {
fragColor = vec4(1.,0.,0.,5.);
    fragColor = .5 * fragColor + .5 * texture(texture_sampler, texture_coord.xy).rgba;
} else {
